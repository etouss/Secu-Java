...
public class Dawg {
    ...
    public boolean searchForStringB(String toSearchFor) {
        ...
        found[0] = false;
        searchForStringRecurse(upperString, 0, (upperString.charAt(0) - 'A' + 1), found);
        ...
        while ( currentIndex != 0 ) {
            if ( currentChar == '?') {
                ...
                if ( (position == (emptyPattern.length() - 1)) ) {
                    if ( nodeEndOfWord(currentIndex) ) {
                        tally[0] += 1;
                        addThisMessage += new String(thePattern, 0, position + 1) + "\n";
                    }
                }
                else {
                    returnHolder = searchForPatternRecurseB(emptyPattern, position + 1, nodeChild(currentIndex), thePattern, tally);
                    addThisMessage += returnHolder;
                }
                currentIndex = nodeNext(currentIndex);
            }
                ...
                thePattern[position] = nodeLetter(currentIndex);
                addThisMessage = searchForPatternRecurseB(emptyPattern, position + 1, nodeChild(currentIndex), thePattern, tally);
                return addThisMessage;
            }
        }
        return addThisMessage;
    ...
    public String[] searchForPatternB(String thisPattern) {
        ...
        if(thisPattern.length()==1){
            ...
            for(char c : uneLettre) {
                if (c == thisPattern.charAt(0) || thisPattern.charAt(0) == '?') res += c+" ";
            }
            return res.split(" ");
        }
        ...
        counter[0] = 0;
        if ( upperString.charAt(0) != '?' ) holder += searchForPatternRecurseB(upperString, 0, (upperString.charAt(0) - '@'), runningPattern, counter);
        else holder += searchForPatternRecurseB(upperString, 0, 1, runningPattern, counter);
